...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.fullName.trim()) { setError('Přezdívka je povinná'); setLoading(false); return }
    if (!formData.phone.trim()) { setError('Telefonní číslo je povinné'); setLoading(false); return }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
          <button onClick={handleGoogleLogin}
            className="w-full flex items-center justify-center gap-3 border border-gray-200 bg-white text-gray-700 py-3 rounded-xl font-semibold hover:bg-gray-50 transition-colors mb-5">
